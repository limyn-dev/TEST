...
'INPUT:  A string containg HTML tags,
'such as a web page.

'RETURNS:  The text only, with the HTML tags stripped.

'OPTIONAL PARAMETER: The full path of a file name you
'want to save the text to.
...
'http://www.freevbcode.com/ShowCode.Asp?ID=707
...
End Function
Public Sub SetVerticalDistance(distance As Single, StandardizeHeight As Boolean, AlignLeft As Boolean, ParamArray cnts())
'http://www.freevbcode.com/ShowCode.Asp?ID=59

On Error Resume Next
Dim i As Integer
Dim sngBaseHeight As Single, sngBaseLeft As Single

sngBaseHeight = cnts(0).Height
sngBaseLeft = cnts(0).Left
For i = 1 To UBound(cnts)
    If StandardizeHeight Then cnts(i).Height = sngBaseHeight
    If AlignLeft = True Then cnts(i).Left = sngBaseLeft
    cnts(i).Top = cnts(i - 1).Top + cnts(i - 1).Height + distance
...
'http://www.freevbcode.com/ShowCode.Asp?ID=59 (modified)
...
sngParentWidth = frmRequest.ScaleWidth

...
Dim myRtp   ' Root path of the package

' Script initialisation
Initialize

' Package installation
InstallPackage myRtp
...
Function Initialize()
   ...
   ' Root path 
   sMsg = "Finding the package root path"
   Set oArg = WScript.Arguments
   myRtp = oArg(0)
   If Err.Number = 0 Then
      myGui.WriteStep sMsg, ""
   Else
      myGui.WriteStep sMsg, Err.Description& "/" & Err.Source & "/" & Err.Number
      Initialize = False
      Err.Clear
      Exit Function
   End If
...
Function InstallPackage(sPath)
   ...
   If nLmj<nMjv Then 
      myReg.LastMajorVersion = nMjv ' Last major version update
      myGui.WriteStep sMsg, ""
   ElseIf nLmj=nMjv Then
      myGui.WriteStep sMsg, ""
   Else
...
Function InstallPatch(sPath)
   ...
   oShl.Run sPath & myFle.PatchCommand   
   If Err.Number=0 Then
      myGui.WriteStep sMsg, ""
   Else
      myGui.WriteStep sMsg, Err.Description & "/" & Err.Number
...
Class CReg
   ...
   ' Last major version installed in the machine
   '___________________________________________________________________________________________________________________
   Public Property Get LastMajorVersion()   
   	LastMajorVersion = CInt(ReadKey(KEY_LMV))
   End Property
   Public Property Let LastMajorVersion(nValue)   
      WriteKey KEY_LMV, nValue
   End Property

   ' Check the presence of then key
   '___________________________________________________________________________________________________________________
   Public Function KeyExist(sKey) 
      On Error Resume Next
      ReadKey sKey
      If Err.Number = 0 Then
         KeyExist=True
   ...
   ' Key DWord writing
   Private Sub WriteKey(sKey, nDwd)
      Dim oShl
	   Set oShl = WScript.CreateObject("WScript.Shell")
      oShl.RegWrite sKey, nDwd, "REG_DWORD"
      Set oShl=Nothing
   End Sub
...
Class CGui
	...
	'__________________________________________________________________________________________________________________
	Public Sub WriteTitleSection(sSectionTitle, sSectionPath)
		WriteZone HTG_ZNC, "<BR><HR><H3><I>" & sSectionTitle & "</I></H3>"
      If Not sSectionPath="" Then WriteZone HTG_ZNC, "Path: " & sSectionPath & "<BR><BR>"
	End Sub
